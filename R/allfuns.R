...
iter.estimator=function(A, B, SigmaUU, SigmaUV, SigmaVV, PleioPThreshold, FDR=FALSE, FDR.alpha=0.05,method="Sidak", Outliers=NULL,warn=TRUE,max.iter=30,eps=1e-4) {
  .dat=subset.all(A, B, SigmaUU, SigmaUV, SigmaVV, Outliers=Outliers)
  Ahat=as.matrix(.dat$A); Bhat=as.matrix(.dat$B); SigmaUU=.dat$SigmaUU; SigmaUV=.dat$SigmaUV; SigmaVV=.dat$SigmaVV
  ...
  while(k<max.iter & thetadiff>(eps*p) & tail(tdd,1)!=0 & length(Outliers)<dim(SigmaUU)[3]) { # times that term because I want it to be sensitive to many phenotypes
    ...
    if(FDR) Outliers=suppressWarnings(which(p.fdr(pleioPs0,just.fdr=TRUE,adjust.method=method)<FDR.alpha)) else Outliers=which(pleioPs0<PleioPThreshold)
...
prepData=function(bT,IVInds=1:nrow(bT$EstHarm),outcome_index=1,verbose=TRUE) {
  ...
  if(verbose) {
    mess1=paste0('Assuming that the outcome is in the ',oi,' index position of biasTerms(<>)$EstHarm, biasTerms(<>)$SEHarm, and biasTerms(<>)$R')
    mess2='You can change this be changing the integer value given to the `oi` argument'
    print(mess1)
    print(' ')
    print(mess2)
    print(' ')
    print('you can silence this message by setting the `verbose` argument to FALSE')
  }
  return(prepDataList)
}
