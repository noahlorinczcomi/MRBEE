...
parser.add_argument('-se',action='store',type=str,help='(Required) comma-separated list of names of BETA columns in GWAS sumstats')
parser.add_argument('-pt',action='store',type=float,help='(Required) P-value threshold. Only SNPs with all GWAS estimates P>this threshold will be considered')
...
print("NOTE: -beta, -snp, and -se flag declarations must be in the corresponding order of -data declarations")
...
def dropRows(Zmat,Zcut):
    ...
    for i in range(0,Zmat.shape[0]):
        if any(abs(Zmat.loc[i,:])>Zcut)==False:
            isKeep.append(i)
    return isKeep
...
print(R)
print('{} SNPs used in correlation matrix estimation'.format(R.shape[0]))
